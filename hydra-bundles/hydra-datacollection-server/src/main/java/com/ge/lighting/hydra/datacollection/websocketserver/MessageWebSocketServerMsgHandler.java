...
@ServerEndpoint(	
        value = "/data/channel",
        decoders = {DataCoder.DataPointDecoder.class},
        encoders = {DataCoder.DataPointEncoder.class}
)
//@Component
public class MessageWebSocketServerMsgHandler {

    private static Logger LOG = LoggerFactory.getLogger(MessageWebSocketServerMsgHandler.class);

    private DataIngestionService dataIngestionService;

    public MessageWebSocketServerMsgHandler() {
        BundleContext context = FrameworkUtil.getBundle(MessageWebSocketServerMsgHandler.class).getBundleContext();
     ...
     */
    @OnMessage
    public void onMessage(DataPoint message, Session session) throws DataUpstreamException {
        LOG.info("Hydra Data-collection service receive DATAPOINT message: " + message);
        this.dataIngestionService.send(message);
    }
